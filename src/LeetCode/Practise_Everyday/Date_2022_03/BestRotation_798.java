 ...
 * @Auther: Lil_boat
 * @Date: 9:34 2022/3/9
 * @Description: 给你一个数组 nums，我们可以将它按一个非负整数 k 进行轮调，
...
public class BestRotation_798 {
    ...
    /*
        思路分析：这道题可能大家都是想着直接移动，然后计算分数，再取最高值。但是A的长度比较大的时候耗时就非常大了。
        ...
        我们可以在这个刚好不得分的k标记一下，通过+1进行标记，这个k就是 (i - A[i] + 1 + N) % N。用一个长度为N的myK数组，
        对于每个元素A[i]，我们都找到其刚好不得分的k = (i - A[i] + 1 + N) % N，那么此时myK[k]就表示数组中的数字在K = k时，A数组中不得分的元素个数。

        可以发现，如果当K = k时，A[i]刚好不得分，当K = k + 1时（左移一个）A[i]继续不得分，但是当K = k + 1时
    ...
    public static int bestRotation(int[] nums) {
        // 数组的长度
        int n = nums.length;
        // resK用于记录K = resK时，此时nums数组中部分的个数最少（即A数组中得分的个数最多）
        int resK = 0;
        // myK[k]代表当 K = k时，nums数组中不得分的个数
        int[] myK = new int[n];
        // 第一步：将nums数组中所有元素都向左移动(i - nums[i] + 1 + n) % n个位置，即K = (i - nums[i] + 1 + n) % n，此时nums[i]刚好不得分
        for (int i = 0; i < n; i++) {
            myK[(i - nums[i] + 1 + n) % n] += 1; // 当K = (i - nums[i] + 1 + n) % n时，不得分的个数自增
        }
        for (int i = 1; i < n; i++) {
            // 当K = (i - nums[i] + 1 + n) % n时，不得分的个数自增
            // 在K = i - 1时不得分的继续不得分，但是当K = i - 1转换到K = i时，处于nums[0]的元素开始得分
            // 更新不得分元素的个数
            myK[i] += myK[i - 1] - 1;
            if (myK[i] < myK[resK]) resK = i; //K = resK时，此时nums数组中部分的个数最少（即nums数组中得分的个数最多）
        }
        return resK;
    }
