...
public class DetectSquares_2013 {
    ...
    由 y 和 ny 可得正方形边长 len，此时再检查唯一确定的两点 (x±len,y) 和 (x±len,ny) 的出现次数，应用乘法原理，即可知道该正方形的方案数，
    统计所有合法方案数即是该询问的答案。
    利用题目范围给定的 x 和 y 具有明确的范围 0 <= x, y <= 1000，我们可以使用数组充当哈希表，
    但是为了拓展性和减少边界判断，即支持将平面拓展到任意大小，最好还是直接使用哈希表。
     */
    // map key 存储x的值 后面一个map中的key存储y的值 value是相同(x,y)的个数
